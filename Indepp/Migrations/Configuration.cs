namespace Indepp.Migrations
{
    ...
    internal sealed class Configuration : DbMigrationsConfiguration<Indepp.DAL.PlaceContext>
    {
        ...
        protected override void Seed(Indepp.DAL.PlaceContext context)
        {
            ...
            places.ForEach(p => context.Places.AddOrUpdate(place => place.ID, p));
            context.SaveChanges();
            ...
            placeDescriptions.ForEach(pd => context.PlaceDescriptions.AddOrUpdate(pd));
            context.SaveChanges();
            ...
            blogPosts.ForEach(bp => context.BlogPosts.AddOrUpdate(blogpost => blogpost.ID, bp));
            context.SaveChanges();
            ...
            articles.ForEach(a => context.Articles.AddOrUpdate(article => article.ID, a));
            context.SaveChanges();
        }
    }
}
